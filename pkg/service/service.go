...
func(s *server) Create(ctx context.Context, req *api.CreateRequest)(*api.CreateResponse, error){
	...
	res, err := c.ExecContext(ctx,
		"INSERT INTO books('title', 'author', 'description','pages','price') VALUES(?,?,?,?,?)",
...
func(s *server) Get(ctx context.Context, req *api.GetRequest)(*api.GetResponse, error){
	...
	rows, err := c.QueryContext(ctx,
		"SELECT 'id', 'title', 'author', 'description', 'pages', 'price' FROM books WHERE 'id'=?", req.Id)
	if err != nil{
	...
	if !rows.Next() {
		...
		return nil, status.Error(codes.NotFound, fmt.Sprintf("ID='%d' id not found",req.Id))
		}
...
func(s *server) GetAll(ctx context.Context, req *api.GetAllRequest)(*api.GetAllResponse, error){
	...
	rows, err := c.QueryContext(ctx, "SELECT 'id', 'title', 'author', 'description','pages' FROM books")
	if err != nil{
		return nil, status.Error(codes.Unknown, "failed to select"+err.Error())
	}
	defer rows.Close()
